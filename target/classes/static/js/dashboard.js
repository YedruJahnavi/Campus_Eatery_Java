...
function renderOrders(orders) {
    ...
    if (activeOrders.length === 0) {
        container.innerHTML = `
            <div class="glass-panel text-center" style="padding: 2.5rem; grid-column: 1/-1; color: var(--text-secondary);">
                <ion-icon name="restaurant-outline" style="font-size: 3rem; margin-bottom: 0.5rem; color: var(--text-3);"></ion-icon>
    ...
    container.innerHTML = activeOrders.map(order => {
        ...
        return `
            <div class="glass-panel" style="padding: 1.5rem; border-left: 4px solid var(--text); display: flex; flex-direction: column; justify-space-between;">
                <div>
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.8rem;">
                        <div>
                            <h3 style="font-size: 1.15rem; margin-bottom: 0.2rem;">Order #${escapeHtml(shortId)}</h3>
                            <span style="font-size: 0.78rem; color: #a1a1aa;">Code: <strong style="color: var(--text);">${escapeHtml(code)}</strong></span>
                        </div>
                        <span style="background: var(--surface-2); color: var(--text); border: 1px solid var(--border-strong); padding: 0.25rem 0.6rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">
                            ${escapeHtml(order.status)}
                        </span>
                    </div>

                    <div style="background: var(--surface-2); border: 1px solid var(--border); padding: 0.8rem; border-radius: 8px; margin-bottom: 1rem;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; font-weight: 600; margin-bottom: 1.2rem;">
                        ...
                        <span style="font-size: 1.2rem; color: var(--text);">₹${grandTotalRupees}</span>
                    </div>
                </div>

                <button class="btn btn-dark btn-block" onclick="updateOrderStatus('${order.id}', '${nextStatus}')">
                    ${actionBtnText}
                </button>
            </div>
        `;
...
function renderMenuItems(items) {
    ...
    tbody.innerHTML = items.map(item => `
        <tr>
            ...
            <td style="color: var(--text); font-weight: 600; font-size: 1.05rem;">
                ₹${((item.pricePaise || 0) / 100).toFixed(2)}
            </td>
            <td>
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="checkbox" ${item.isAvailable ? 'checked' : ''} style="width: 18px; height: 18px; accent-color: var(--text); cursor: pointer;" onchange="toggleItemAvailability('${item.id}', this.checked)">
                    <span style="color: var(--text); font-weight: 600; font-size: 0.85rem;">
            ...
            <td>
                <div style="display: flex; gap: 0.5rem;">
                    <button class="btn btn-outline btn-sm" onclick='openModal(${JSON.stringify(item).replace(/'/g, "&apos;")})'>
                        <ion-icon name="create-outline"></ion-icon> Edit
                    </button>
                    <button class="btn btn-outline btn-sm" style="color: var(--text); border-color: var(--border-strong);" onclick="deleteMenuItem('${item.id}')">
